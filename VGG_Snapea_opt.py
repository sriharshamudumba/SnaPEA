...
class OptimizedSnaPEALayer(nn.Module):
    ...
    def forward(self, x):
        if isinstance(self.layer, nn.Conv2d):
            ...
            if x.numel() > 1e6:  # Arbitrary large size threshold
                chunk_size = int(x.numel() // 1e6) + 1  # Divide into manageable chunks
                thresholds = []
                for chunk in x.chunk(chunk_size):
                    thresholds.append(torch.quantile(chunk.abs(), self.percentile / 100.0))
                threshold = torch.tensor(thresholds).mean()  # Use average threshold
            else:
...
def main():
    ...
    # Load VGG model and wrap with SnaPEA
